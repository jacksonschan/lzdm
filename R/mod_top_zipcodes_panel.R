...
mod_top_zipcodes_panel_server <- function(input, output, session,r){
  ...
  data <- reactive({
    ...
    d <- zip_dataset@data %>%
      ...
      dplyr::select(.,GEOID10,name,home_prices,yoy) %>%
      dplyr::filter(., home_prices >= r$user_inputs_server$HomePrices[1] 
                    , home_prices <= r$user_inputs_server$HomePrices[2]
                    , yoy >= r$user_inputs_server$YoY[1]/100
                    , yoy <= r$user_inputs_server$YoY[2]/100
      ) 
  })
  ...
  output$ziptable <- DT::renderDataTable({
    ...
    , colnames = c('Zip', 'Name', 'Home Value', '1YR Change')
    , rownames = FALSE
    , selection = 'single'
    ) %>% 
      DT::formatCurrency(columns=c('home_prices'),digits = 0) %>%
      DT::formatPercentage(columns=c('yoy'))
  })
